...
var AddressBehaviors = require('js/address.js')
var app = getApp()
Page({
    ...
    behaviors: [app.configBehaviors, AddressBehaviors],
    ...
    onLoad: function (options) {
        this.setData({
            ...
            CustomerTakeType: options.CustomerTakeType || 1,
        })
        this.onInit()
    },
    ...
    // 获取门店信息
    async getStore(){
        var res = await app.db.product.productGetStoreInfo({ ShopId: this.data.shopID })
        this.setData({ store:res.data })

        var res = await app.db.product.productGetStoreList({  ShopId: this.data.shopID  })
    ...
    // 切换门店
    changeStore(e){
        var store = this.data.storeList[e.detail.value]
        this.setData({
            store: store,
            shopID: store.Id,
        })
     ...
     */
    async getOrderPrice(){       
        var res = await app.db.product.productGetPrice({ 
     ...
     */
    async getOrderGen(){
        var res = await app.db.product.productGen({
            jsondata: this.getJson()
        })
        ...
        var res = await app.db.product.productFinish({
            OrderId: OrderId,
